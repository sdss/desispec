...
def findfile(filetype, night=None, expid=None, camera=None, groupname=None,
    ...
    Options:
        rawdata_dir : overrides $LVM_SPECTRO_DATA
        specprod_dir : overrides $LVM_SPECTRO_REDUX/$SPECPROD/
        download : if not found locally, try to fetch remotely
        outdir : use this directory for output instead of canonical location
    """

    #- NOTE: specprod_dir is the directory $LVM_SPECTRO_REDUX/$SPECPROD,
...
def get_raw_files(filetype, night, expid, rawdata_dir=None):
    ...
    Args:
        ...
        rawdata_dir(str): [optional] overrides $LVM_SPECTRO_DATA
...
def rawdata_root():
    """Returns directory root for raw data, i.e. ``$LVM_SPECTRO_DATA``
    ...
    return os.environ['LVM_SPECTRO_DATA']


def specprod_root():
    """Return directory root for spectro production, i.e.
    ``$LVM_SPECTRO_REDUX/$SPECPROD``.
    ...
    return os.path.join(os.getenv('LVM_SPECTRO_REDUX'), os.getenv('SPECPROD'))
