...
class Tile(Base):
    ...
    def brick_offset(self, brick):
        ...
        Parameters
        ----------
        brick : :class:`~lvmspec.database.metadata.Brick`
            A brick.
    ...
    def _coarse_overlapping_bricks(self, session):
        ...
        -------
        :class:`list`
            A list of :class:`~lvmspec.database.metadata.Brick` objects.
        """
    ...
    def simulate_frame(self, session, band, spectrograph,
        ...
        -------
        :class:`tuple`
            A tuple containing a :class:`~lvmspec.database.metadata.Frame` object
...
class FrameStatus(Base):
    ...
    :class:`~lvmspec.database.metadata.Frame`.
    """
...
class BrickStatus(Base):
    """Representation of the status of a particular
    :class:`~lvmspec.database.metadata.Brick`.
    """
...
def main():
    ...
    prsr.add_argument('-d', '--data', action='store', dest='datapath',
                      default=os.path.join(os.environ['LVM_SPECTRO_SIM'],
                                           os.environ['SPECPROD']),
