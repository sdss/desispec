    ...
    if entry is True :
        ...
        if keyword in calibration_data :
            ...
            if calibration_data_path is None :
                error_message = "LVM_CCD_CALIBRATION_DATA environment variable must be set in order to find the calibration file %s"%basefilename
...
def preproc(rawimage, header, primary_header, bias=True, dark=True, pixflat=True, mask=True, bkgsub=False, nocosmic=False, cosmics_nsig=6, cosmics_cfudge=3., cosmics_c2fudge=0.8,ccd_calibration_filename=None, nocrosstalk=False, nogain=False):
    ...
    if ccd_calibration_filename is None :
        ...
        if not resource_exists('lvmspec', srch_file):
            ...
            ccd_calibration_filename=resource_filename('lvmspec', srch_file)
    ...
    if "LVM_CCD_CALIBRATION_DATA" in os.environ :
        calibration_data_path = os.environ["LVM_CCD_CALIBRATION_DATA"]
    else :
...
#-------------------------------------------------------------------------
#- The following I/O routines are here instead of lvmspec.io to avoid a
