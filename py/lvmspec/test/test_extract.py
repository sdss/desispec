...
import lvmspec.io
import lvmspec.scripts.extract
...
class TestExtract(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        lvmspec.io.write_image(cls.imgfile, img, meta=dict(flavor='science'))

        fibermap = lvmspec.io.empty_fibermap(100)
        lvmspec.io.write_fibermap(cls.fibermapfile, fibermap)
    ...
    def test_extract(self):
        ...
        opts = cmd.split(" ")[1:]
        args = lvmspec.scripts.extract.parse(opts)
        lvmspec.scripts.extract.main(args)

        self.assertTrue(os.path.exists(self.outfile))
        frame1 = lvmspec.io.read_frame(self.outfile)
        model1 = fits.getdata(self.outmodel)
        os.remove(self.outfile)
        os.remove(self.outmodel)

        lvmspec.scripts.extract.main_mpi(args, comm=None)
        self.assertTrue(os.path.exists(self.outfile))
        frame2 = lvmspec.io.read_frame(self.outfile)
