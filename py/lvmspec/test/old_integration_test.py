...
python -m lvmspec.test.integration_test
"""
...
def check_env():
    ...
    if 'LVM_BASIS_TEMPLATES' not in os.environ:
        log.warning('missing $LVM_BASIS_TEMPLATES needed for simulating spectra')
        missing_env = True
    ...
    for name in (
        'LVM_SPECTRO_SIM', 'LVM_SPECTRO_REDUX', 'PIXPROD', 'SPECPROD'):
    ...
    os.environ['LVM_SPECTRO_DATA'] = os.path.join(os.getenv('LVM_SPECTRO_SIM'), os.getenv('PIXPROD'))
...
def integration_test(night=None, nspec=5, clobber=False):
    ...
    for expid, program in enumerate(programs):
        for ic, channel in enumerate(channels):
            ...
            psffile = '{}/data/specpsf/psf-{}.fits'.format(os.getenv('LVMMODEL'), channel)
    ...
    if 'STD_TEMPLATES' in os.environ:
        ...
        std_templates = os.getenv('LVM_ROOT')+'/spectro/templates/star_templates/v1.1/star_templates_v1.1.fits'
    ...
    options = get_options('--clobber', '--filename', 'dailytest.db',
                          os.path.join(os.environ['LVM_SPECTRO_REDUX'],
                                       os.environ['SPECPROD']))
