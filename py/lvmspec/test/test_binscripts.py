...
import lvmspec.scripts
import lvmspec.scripts.sky
import lvmspec.scripts.fiberflat
import lvmspec.scripts.fluxcalibration
...
class TestBinScripts(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not os.path.isdir(cls.binDir):
            raise RuntimeError('Unable to auto-locate lvmspec/bin from {}'.format(__file__))
    ...
    def test_compute_fiberflat(self):
        ...
        err = runcmd(lvmspec.scripts.fiberflat.main, args=[args,],
    ...
    def test_compute_fluxcalib(self):
        ...
        #- Remove outputs and call again via function instead of system call
        self._remove_files(outputs)
        args = lvmspec.scripts.fluxcalibration.parse(cmd.split()[2:])
        err = runcmd(lvmspec.scripts.fluxcalibration.main, args=[args,],
            inputs=inputs, outputs=outputs, clobber=True)
    ...
    def test_compute_sky(self):
        ...
        err = runcmd(lvmspec.scripts.sky.main, args=[args,],
            inputs=inputs, outputs=outputs, clobber=True)
        self.assertEqual(err, None)
