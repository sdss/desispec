"""
tests for Quicklook QA class and functions. It also indludes tests on low level functions on lvmspec.qa.qalib
"""
...
from lvmspec.qa import qa_quicklook as QA
...
class TestQL_QA(unittest.TestCase):
    ...
    def setUp(self):
        ...
        #self.psffile=os.environ['LVMMODEL']+'/data/specpsf/psf-b.fits'
        self.config={"kwargs":{
            "refKey":None,
            "param":{},
            "qso_resid":None
        ...
        lvmspec.io.write_raw(self.rawfile,rawimg,hdr,camera=self.camera)
        self.rawimage=fits.open(self.rawfile)

        #- read psf, should use specter.PSF.load_psf instead of lvmspec.PSF(), otherwise need to create a psfboot somewhere.
        ...
        self.image = lvmspec.image.Image(img_pix, img_ivar, img_mask, camera='z1',meta=hdr)
        lvmspec.io.write_image(self.pixfile, self.image)

        self.fibermap = lvmspec.io.empty_fibermap(30)
        ...
        lvmspec.io.write_fibermap(self.fibermapfile, self.fibermap)
        ...
        self.frame=lvmspec.frame.Frame(wave,flux,ivar,resolution_data=resolution_data,fibermap=self.fibermap)
        ...
        lvmspec.io.write_frame(self.framefile, self.frame)

        #- make a skymodel
        sky=np.ones_like(self.frame.flux)*0.5
        skyivar=np.ones_like(sky)
        self.mask=np.zeros(sky.shape,dtype=np.uint32)
        self.skymodel=lvmspec.sky.SkyModel(wave,sky,skyivar,self.mask)
        self.skyfile=lvmspec.io.write_sky(self.skyfile,self.skymodel)
    ...
    def test_sky_resid(self):
        ...
        skym1=lvmspec.sky.SkyModel(self.frame.wave,self.skymodel.flux,self.skymodel.ivar,self.mask)
        skym2=lvmspec.sky.SkyModel(self.frame.wave,self.skymodel.flux*0.5,self.skymodel.ivar,self.mask)
        frame1=copy.deepcopy(self.frame)
        frame2=copy.deepcopy(self.frame)
        lvmspec.sky.subtract_sky(frame1,skym1)
        lvmspec.sky.subtract_sky(frame2,skym2)
    ...
    def testSignalVsNoise(self):
        ...
        lvmspec.sky.subtract_sky(thisframe,self.skymodel)
        ...
        nullfibermap=lvmspec.io.empty_fibermap(10)
    ...
    def testCalcXWSigma(self):
        ...
        xwimage = lvmspec.image.Image(xw_img_pix, xw_img_ivar, xw_img_mask, camera='z1',meta=xw_hdr)
        lvmspec.io.write_image(self.xwfile, xwimage)
