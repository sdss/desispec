...
python -m lvmspec.test.integration_test_quicklook
"""
...
def parse(options=None):
    ...
    for name in (
        'LVM_SPECTRO_SIM', 'PIXPROD', 'LVMMODEL'):
    ...
    if missing_env:
        ...
        log.warning("    PSF files are found in $LVMMODEL")
    ...
    sim_dir = os.path.join(os.environ['LVM_SPECTRO_SIM'],os.environ['PIXPROD'],args.night)
...
def sim(night,nspec):
    ...
    psf_b = os.path.join(os.environ['LVMMODEL'],'data','specpsf','psf-b.fits')
    psf_r = os.path.join(os.environ['LVMMODEL'],'data','specpsf','psf-r.fits')
    psf_z = os.path.join(os.environ['LVMMODEL'],'data','specpsf','psf-z.fits')

    #- Create files needed to run quicklook
    sim_dir = os.path.join(os.environ['LVM_SPECTRO_SIM'],os.environ['PIXPROD'],night)
...
def integration_test(args=None):
    ...
    configfile=resource_filename('lvmspec','data/quicklook/qlconfig_dark.yaml')
    for camera in ['r0','z0']:
