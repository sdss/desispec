...
def parse(options=None):
    ...
    parser.add_argument("--data", required=False, default=None, help="override LVM_SPECTRO_DATA")
    parser.add_argument("--redux", required=False, default=None, help="override LVM_SPECTRO_REDUX")
...
def main(args):
    ...
    rawdir = args.data
    if rawdir is None:
        if "LVM_SPECTRO_DATA" in os.environ:
            rawdir = io.rawdata_root()
        else:
            raise RuntimeError("You must set LVM_SPECTRO_DATA in your environment or use the --raw commandline option")
    else:
        rawdir = os.path.abspath(rawdir)
        os.environ["LVM_SPECTRO_DATA"] = rawdir
    ...
    specdir = args.redux
    if specdir is None:
        if "LVM_SPECTRO_REDUX" in os.environ:
            specdir = os.environ["LVM_SPECTRO_REDUX"]
            proddir = io.specprod_root()
        else:
            raise RuntimeError("You must set LVM_SPECTRO_REDUX in your environment or use the --redux commandline option")
    else:
        specdir = os.path.abspath(specdir)
        proddir = os.path.join(specdir, prodname)
        os.environ["LVM_SPECTRO_REDUX"] = specdir

    # Check LVMMODEL

    if "LVMMODEL" not in os.environ:
        raise RuntimeError("You must set LVMMODEL in your environment")

    # Check LVM_ROOT

    if "LVM_ROOT" not in os.environ:
        raise RuntimeError("You must set LVM_ROOT in your environment")
    desiroot = os.environ["LVM_ROOT"]
    ...
    with open(setupfile, "w") as s:
        ...
        s.write("export LVM_SPECTRO_DATA={}\n".format(rawdir))
        s.write("export LVM_SPECTRO_REDUX={}\n".format(specdir))
        s.write("export SPECPROD={}\n".format(prodname))

        s.write("\n")
        if args.debug:
            s.write("export LVM_LOGLEVEL=\"DEBUG\"\n\n")
        else:
            s.write("#export LVM_LOGLEVEL=\"DEBUG\"\n\n")
        ...
        for nt in nights:
            ...
            for band in ["b", "r", "z"]:
                for spec in range(10):
                    ...
                    target = os.path.join(os.environ["LVMMODEL"], "data", "specpsf", "psf-{}.fits".format(band))
